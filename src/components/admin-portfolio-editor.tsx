...
type Section = "content" | "brand" | "projects" | "messages";
...
export function AdminPortfolioEditor() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      supabase.from("site_settings").select("key, value").in("key", ["headshot_url", "headshot_zoom", "headshot_shift_y"]),
  ...
  async function saveBrand(event: FormEvent<HTMLFormElement>) {
    ...
    const [themeResult, headshotResult, zoomResult, shiftResult] = await Promise.all([
      ...
      supabase.from("site_settings").update({ value: headshotUrl, updated_at: updatedAt, updated_by: user.id }).eq("key", "headshot_url").select("key").single(),
      supabase.from("site_settings").update({ value: String(headshotZoom), updated_at: updatedAt, updated_by: user.id }).eq("key", "headshot_zoom").select("key").single(),
      supabase.from("site_settings").update({ value: String(headshotShiftY), updated_at: updatedAt, updated_by: user.id }).eq("key", "headshot_shift_y").select("key").single(),
    ]);
    const error = themeResult.error || headshotResult.error || zoomResult.error || shiftResult.error;
  ...
  return (
    <div className="admin-studio">
      ...
      <div className="admin-tabs" role="tablist" aria-label="Portfolio settings">
        {(["content", "brand", "projects", "messages"] as Section[]).map((item) => <button key={item} type="button" role="tab" aria-selected={section === item} className={section === item ? "is-active" : ""} onClick={() => { setSection(item); setMessage(""); }}>{item === "content" ? "Homepage copy" : item === "brand" ? "Brand & images" : item === "projects" ? "Projects" : `Messages (${contactMessages.length})`}</button>)}
      </div>
      ...
      {section === "brand" && <form className="admin-panel admin-form" onSubmit={saveBrand}><div className="panel-heading"><div><p className="eyebrow">Visual identity</p><h2>Colors & headshot</h2></div><button className="text-button" type="button" onClick={() => { setSettings((current) => ({ ...current, ...Object.fromEntries(COLOR_FIELDS.map((field) => [field, DEFAULT_PORTFOLIO_SETTINGS[field]])) })); applyTheme(DEFAULT_PORTFOLIO_SETTINGS); }}>Reset preview</button></div><p className="panel-note">Color and image framing changes preview live on this page. Save when the combination feels right.</p><div className="color-grid">{COLOR_FIELDS.map((field) => <label className="color-field" key={field}><span><input type="color" value={settings[field]} onChange={(e) => { const next = { ...settings, [field]: e.target.value }; setSettings(next); applyTheme(next); }} /><strong>{colorLabels[field]}</strong></span><input className="hex-input" value={settings[field]} pattern="#[0-9A-Fa-f]{6}" onChange={(e) => { const value = e.target.value; updateSetting(field, value); if (/^#[0-9A-Fa-f]{6}$/.test(value)) applyTheme({ ...settings, [field]: value }); }} required /></label>)}</div><div className="field-section"><h3>Profile image</h3><div className="image-url-layout"><div className="admin-headshot-preview"><img src={headshotUrl || DEFAULT_HEADSHOT_URL} alt="Headshot preview" style={{ transform: `translateY(${headshotShiftY}%) scale(${headshotZoom / 100})` }} /></div><div className="headshot-controls"><label>Headshot URL<input type="url" value={headshotUrl} onChange={(e) => setHeadshotUrl(e.target.value)} required /></label><div className="image-range-grid"><label><span>Zoom <output>{headshotZoom}%</output></span><input type="range" min="100" max="200" step="1" value={headshotZoom} onChange={(e) => setHeadshotZoom(Number(e.target.value))} /></label><label><span>Shift up or down <output>{headshotShiftY === 0 ? "Centered" : `${Math.abs(headshotShiftY)}% ${headshotShiftY < 0 ? "up" : "down"}`}</output></span><input type="range" min="-30" max="30" step="1" value={headshotShiftY} onChange={(e) => setHeadshotShiftY(Number(e.target.value))} /></label></div><button className="text-button framing-reset" type="button" onClick={() => { setHeadshotZoom(100); setHeadshotShiftY(0); }}>Reset image framing</button></div></div></div><button className="button button-primary" disabled={busy} type="submit">{busy ? "Saving…" : "Save brand settings"}</button></form>}
