...
export function AdminResumeEditor({ user, onStatus }: Props) {
  ...
  function updateLeadership(index: number, values: Partial<ResumeExperience>) {
    setContent((current) => ({ ...current, leadership: current.leadership.map((entry, entryIndex) => entryIndex === index ? { ...entry, ...values } : entry) }));
  }

  function addLeadership() {
    setContent((current) => ({ ...current, leadership: [...current.leadership, { title: "", organization: "", date: "", bullets: [] }] }));
  }

  function removeLeadership(index: number) {
    setContent((current) => ({ ...current, leadership: current.leadership.filter((_, entryIndex) => entryIndex !== index) }));
  }
  ...
  return (
    <form className="admin-panel admin-form" onSubmit={save}>
      ...
      <div className="field-section"><h3>Leadership</h3><label>Public section title<input value={content.sectionTitles.leadership} onChange={(event) => setContent({ ...content, sectionTitles: { ...content.sectionTitles, leadership: event.target.value } })} required /></label>{content.leadership.map((entry, index) => <fieldset className="resume-admin-entry" key={index}><legend>Leadership item {index + 1}</legend><button className="text-button resume-project-remove" type="button" onClick={() => removeLeadership(index)}>Remove item</button><div className="field-pair"><label>Title<input value={entry.title} onChange={(event) => updateLeadership(index, { title: event.target.value })} required /></label><label>Organization or context<input value={entry.organization} onChange={(event) => updateLeadership(index, { organization: event.target.value })} required /></label></div><label>Date or timeframe<input value={entry.date} onChange={(event) => updateLeadership(index, { date: event.target.value })} /></label><label>Bullet points<textarea rows={6} value={toLines(entry.bullets)} onChange={(event) => updateLeadership(index, { bullets: fromLines(event.target.value) })} required /></label></fieldset>)}<button className="button button-quiet admin-add-button" type="button" onClick={addLeadership}>+ Add leadership item</button></div>
